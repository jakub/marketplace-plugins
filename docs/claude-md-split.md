...
Migrating: install the plugin, move the engineering and workflow content out of the personal file (keep a dated backup), add the pointer, start a fresh session, and compare `/flow charter` against what got injected. To change the charter, edit `plugins/flow/charter/charter.md` in your clone, push, and reinstall. It runs about 150 lines, and every one of them is read in every session, so anything that isn't true in every session belongs in a command body or the skill instead. Size matters mechanically too: Claude Code caps a hook's output at 10,000 characters, so the plugin delivers the charter as two hooks and warns when a half gets close.
