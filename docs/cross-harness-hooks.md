...
| Flow prep stage | `/flow:prep` invokes the `prep` skill directly; there is no command alias | The plugin-namespaced `prep` skill, named by the human per the charter; `agents/openai.yaml` contains display metadata only | `skills/prep/SKILL.md` - one body, one `## Host mechanics` section, one subsection per host |
| Flow issue stage | `/flow:issue` invokes the `issue` skill directly; there is no command alias | The plugin-namespaced `issue` skill, named by the human per the charter; `agents/openai.yaml` contains display metadata only | `skills/issue/SKILL.md` - one body, one `## Host mechanics` section, one subsection per host |
| Flow land stage | `/flow:land` invokes the `land` skill directly; there is no command alias | The plugin-namespaced `land` skill, named by the human per the charter; `agents/openai.yaml` contains display metadata only | `skills/land/SKILL.md` - one body, one `## Host mechanics` section, one subsection per host |
