...
| Flow prep stage | `/flow:prep` invokes the `prep` skill directly; there is no command alias | The plugin-namespaced `prep` skill, with `agents/openai.yaml` setting `allow_implicit_invocation: false` | `skills/prep/SKILL.md` - one body, one `## Host mechanics` section, one subsection per host |
| Flow issue stage | `/flow:issue` invokes the `issue` skill directly; there is no command alias | The plugin-namespaced `issue` skill, with `agents/openai.yaml` setting `allow_implicit_invocation: false` | `skills/issue/SKILL.md` - one body, one `## Host mechanics` section, one subsection per host |
| Flow land stage | `/flow:land` invokes the `land` skill directly; there is no command alias | The plugin-namespaced `land` skill, with `agents/openai.yaml` setting `allow_implicit_invocation: false` | `skills/land/SKILL.md` - one body, one `## Host mechanics` section, one subsection per host |
