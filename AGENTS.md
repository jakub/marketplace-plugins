...
`hooks/scripts/inject-unslop.mjs` delivers the rules, not the skill description - "Must always apply" in frontmatter is advice the model follows unreliably. One script, two modes, selected by the argument in `hooks.json`. `session` runs on SessionStart and prints to stdout. `subagent` runs on SubagentStart, reads the hook JSON on stdin, and answers with `hookSpecificOutput.additionalContext`.
