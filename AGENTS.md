...
Codex uses `hooks/codex.json` and one `inject-charter-codex.mjs` handler. Its context limit is large enough to keep the current charter inline, and one handler preserves order. Flow's shared `lib/hook-policy.mjs` owns protected-file and publication policy; Claude and Codex adapters own their different tool inputs and output decisions. Never return Claude's publication `ask` result to Codex: Codex treats that unsupported value as a hook failure and lets the command continue, so its adapter must deny and direct the human to publish manually.
