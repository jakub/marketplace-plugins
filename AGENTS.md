...
Gripe's Codex checkpoint state is folded from structured PostToolUse events, never the unstable Codex transcript format. Codex does not identify the subagent actor on PostToolUse, so the checkpoint is parent-session state evaluated on main Stop only. Do not add a Codex SubagentStop checkpoint until the event stream can attribute its evidence. PermissionDenied and StopFailure remain Claude-only because Codex has no after-the-fact equivalents; a nearby event with different meaning is not a compatibility layer.
