...
Facts that go stale (model pricing, the Codex App Server protocol) carry an as-of date. Re-verify anything older than a quarter. The delegation contract lives in `plugins/flow/docs/DELEGATION.md` and `plugins/flow/src/delegation/app-server.mjs`, validated against Codex CLI 0.150.1 as of 2026-08-26. Two smokes cover it. `scripts/smoke-delegation.mjs` drives the service against a fake App Server, so it needs no Codex, no network, and no installed dependencies; run it anywhere. `scripts/smoke-bundle-drift.mjs` rebuilds `dist/delegation.mjs` from `src/delegation` and fails unless the committed bundle comes out byte-identical, which means it only works in a dev tree with `npm ci` already run in `plugins/flow/deps`. Every install runs the bundle and never the source, so an edit under `src/delegation` is half a change until `npm run build` in `plugins/flow/deps` regenerates `dist/delegation.mjs` and both go in the same commit.
