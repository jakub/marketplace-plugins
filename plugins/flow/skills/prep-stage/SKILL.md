...
**Dialectic legs.** Native: `spawn_agent` on `gpt-5.6-sol` at high effort, `fork_turns: "none"`, read-only by its prompt. Bridge: `delegate_to_claude` with `claude-opus-5` at xhigh effort, or `claude-fable-5-1` for user-facing UI or copy, `access: read-only`, `delivery: attached`, the repository root as `cwd`. The delegation tools take the provider's own model ids, not the charter table's short names. The outside opinion here is a Claude model: the charter's "Sol is the decorrelated seat" is written from a Claude host.
