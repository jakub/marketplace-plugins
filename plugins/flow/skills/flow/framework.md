...
| gpt-5.6-sol | external decorrelation: outside design opinion, adversarial review, general delegation. reach it via the **codex-delegate** agent or `/codex:*` — never **codex-rescue** (write-capable by default, returns nothing on failure) | `high` — codex config default (`~/.codex/config.toml`, as of 2026-07); pin `--model`/`--effort` per call to override | taste-critical surfaces; codex-reviewing-codex |
