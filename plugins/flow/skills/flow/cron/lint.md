...
3. Propose a `ready-for-agent` issue that fails the contract in `${CLAUDE_PLUGIN_ROOT}/skills/flow/label-contract.md` for demotion via `node ${CLAUDE_PLUGIN_ROOT}/scripts/lint-actions.mjs demote-unready <repo> <issue-number> <the failed contract point>`. The executor re-derives that the issue is open, carries `ready-for-agent` alone, and has no live branch, worktree, claim tag, or open PR, holds the claim tag while it edits, moves it to `needs-triage`, and comments with the point you gave it.
...
5. Propose an open issue with no lifecycle label for triage via `node ${CLAUDE_PLUGIN_ROOT}/scripts/lint-actions.mjs triage-unlabelled <repo> <issue-number>`; it adds `needs-triage` and comments.

There is no `gh issue edit` on this job's allowlist: a label moves only through those three verbs, and a refusal from any of them is a report line. Comments you write yourself are one paragraph, signed `- flow nightly lint`.

Never: create issues or PRs, push, force anything, run mutating git directly, edit files, or chain an executor with `;`, `&&` or `||` (the guard refuses a plugin script joined to another command; a pipe into a filter is fine).
