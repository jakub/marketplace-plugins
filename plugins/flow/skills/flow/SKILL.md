...
7. **Known flakes**: create `.github/known-flakes.txt` (empty). `/flow:land` reads it - one entry per line, either a bare CI check name or `check-name:test_name` for a single flaky test (`land.md` documents both forms), naming what the repo consciously merges through. Lore lives in the repo, not in command prompts.
...
- **recovery**: `/flow:issue` has no runId; a fresh session reconstructs the run from the issue's event journal plus the worktree diff. The deprecated fixed pipeline has its own resume path (`issue-fixed.md` § Recovery).

## The deprecated fixed pipeline

`/flow:issue-fixed` conducts `workflows/issue-fixed.mjs`, a fixed workflow kept as a fallback and as a parts library for ad-hoc Workflow scripts (report salvage, envelope rules, schemas, push-verify prompts). The script is the only stage-level description of what it does - read it rather than a summary. The dynamic run (`/flow:issue`) composes its own fabric per issue instead.
