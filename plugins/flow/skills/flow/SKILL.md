...
2b. Delegation entrypoint, once per machine before the first Codex Flow session. Flow must already be installed in Codex. Resolve that installed Codex package root, even when setup runs from Claude Code, and run `node <codex-plugin-root>/scripts/install-delegate.mjs install` yourself. The Claude cache and a source checkout are not valid installer roots. It installs the stable `flow-delegate` command on PATH and registers the installed Flow package cache directory and layout for the canonical Codex home. This setup action authorizes the install; use the host's normal approval mechanism if the destination requires it. Check the install result and command resolution. Do not hand the human a per-issue shell command.

   Codex starts MCP before SessionStart, so the hook cannot guarantee the first launch. Complete this install before the next project session, or explicitly reload the app's MCP servers afterward. SessionStart idempotently maintains the registration on later starts. Versioned plugin upgrades under that registered directory select the exact version named in the MCP definition before any hook runs. A local cache is accepted only when its manifest matches the requested version. If hooks are disabled or untrusted, setup is still required when the registration is missing. Changing the registered package directory or cache layout requires an explicit uninstall first. `node <codex-plugin-root>/scripts/install-delegate.mjs uninstall` removes this home's registration when removal is requested, including when the old package has been removed. There is no per-issue launcher, parent-directory grant or routine session restart.

   The dispatcher requires user-owned cache inputs and installation paths without group or world write permission. Configure the Codex host to use a restrictive umask such as `022` before plugin installation and future upgrades. If existing paths fail this check, inspect the path named by the diagnostic before removing its group/world write permission. The installer never changes unrelated permissions. HOME and CODEX_HOME aliases resolve to their canonical directories; symlinked child paths such as `~/.local` remain refused. `docs/DELEGATION.md` records these setup requirements.

2c. **Machine layer**, once per machine: `bash <plugin root>/scripts/install-cron.sh status`; if the launcher is missing, run `install`. This arms the nightly lint and the weekly doc sweep; **Ambient machinery** says what they need. Skip on a machine without systemd user sessions and say so.
...
- The delegation entrypoint maintenance hook registers the installed package cache directory for the current Codex home and maintains `flow-delegate` on PATH. Exact version selection works across versioned upgrades without waiting for this hook. The setup action installs it before the first MCP launch. Hook maintenance cannot bootstrap that earlier launch.
