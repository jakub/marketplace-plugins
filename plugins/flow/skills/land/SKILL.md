...
It re-reads the PR state and base, the open PRs stacked on this one, every top-level comment, every review thread over paged GraphQL, a `## follow-up draft` comment, and the auto-merge and merge-queue state. It reads every check run and commit status on the head and partitions them against the base branch's `.github/known-flakes.txt`, diffing the PR's own copy of that file against it. It collects the linked issues three ways: parsed by GitHub, recovered from the branch name and closing phrases, and bare mentions. It prints one JSON object with `verdict`, `stops` and `attention`. Exit 0 is `pass`, 1 is `stop`, 4 is a read that failed, which is never a pass, and 2 is a usage refusal with no JSON at all: a bad argument, an origin remote with no host or with a port, query string or fragment, an origin host that is not `github.com` and not in the `FLOW_GH_HOSTS` environment allowlist, or an `--accept-flake` the base allowlist does not declare or that names an ambiguous check. Read its stderr and fix the call.
