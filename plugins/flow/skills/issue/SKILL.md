...
All work happens in a worktree at `../<repo>-issue-N-<slug>`, a sibling of the repository, on branch `feat|fix|chore/issue-$N-<slug>`, based at the head the claim verified. The executor creates the branch and fills the worktree. A host that fixes its writable roots at startup may first reserve that exact path as an empty directory so the session can grant it and nothing broader.

**Workspace boundary.** The worktree has to sit inside the boundary the host enforces for this session, and your host's subsection says how to prove that. First run `node <plugin-root>/scripts/issue-claim.mjs plan <N>` from the repository root. It performs the claim's read-only issue, path and repository checks and prints the exact `repoRoot`, `worktree`, `branch` and `acDigest` without creating anything. The executor checks what it can from git alone: the repository's common git directory resolves under the repository's parent, that parent is writable, and an existing target is a real empty directory rather than a link or occupied path. A prospective path outside the session's boundary is a stop before the first mutation. The bridge runs its own workspace check on every call, and the delegate skill says how to read that failure; passing it says a delegation job may run there, never that your own shell or a native seat may write there.
...
A failed preflight stops the run and writes nothing: no label, no comment, no ping. Nothing has been claimed, no seat is running, and the human who named this issue is still here, so the report goes to them in the turn - what failed (the drift status, or the workspace boundary the worktree would fall outside) and the exact entry point to relaunch from, spelled the way your host's subsection spells it. `needs-human` is for a block found mid-run, where a durable marker is the only way the human learns of it; a stage that never started needs no marker, and the label would make the relaunch this stop just recommended fail the claim of §3. No inline edits happen for the rest of the run either: writing the code yourself because no seat can be spawned is the fallback this preflight exists to catch.
...
Run `node <plugin-root>/scripts/issue-claim.mjs claim <N>` from the repository root, once. It scans for a live run (local worktrees and branches, the issue's branches on origin, open PRs), accepts only an absent target or the exact empty directory prepared by the host launcher, takes the tag, scans and checks the target again while holding it, creates the worktree and the branch at origin's verified main, confirms Git registered that exact path and branch, pushes the branch, moves the issue to assigned plus `in-progress`, and releases the tag once the branch reads back on origin. It prints one JSON line, and its `result` is the whole answer:
...
**Workspace boundary.** Codex fixes writable roots before this stage creates its worktree, and its MCP client advertises no roots. The supported entry point is `node <plugin-root>/scripts/codex-issue.mjs <N>` from the repository. It runs the read-only plan, reserves only the planned empty directory, and launches `codex -C <repoRoot> --add-dir <worktree>` with `CODEX_PROJECT_DIR` set to that worktree. The session can then write the repository and this one worktree, while Flow's MCP server accepts delegated jobs only in the worktree. It never receives the directory that holds the user's other repositories. At preflight, require the plan's `repoRoot` to be the session cwd, its `worktree` to be an exact additional writable root supplied by the host, and `CODEX_PROJECT_DIR` to equal that same path. Run `delegation_doctor` for the planned worktree. Any missing or broader root is a stop with the exact launcher command. Do not claim the issue first, move the worktree inside the repository, or relaunch from the repository's parent.
