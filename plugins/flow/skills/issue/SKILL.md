...
This runs before any mutation. No assignment, label, comment, claim tag, branch, worktree or spawn happens until it passes, and a failure leaves the issue exactly as it found it. A host that cannot run this stage must never stamp `in-progress` on an issue and walk away.
...
A failed preflight stops the run and writes nothing: no label, no comment, no ping. Nothing has been claimed, no seat is running, and the human who named this issue is still here, so the report goes to them in the turn. It names what failed and which project root or setup prerequisite to correct before retrying the same issue request. `needs-human` is for a block found mid-run, where a durable marker is the only way the human learns of it; a stage that never started needs no marker, and the label would make the relaunch this stop just recommended fail the claim of §3. Do not fall back to writing the code yourself because no seat can be spawned. That fallback is what this preflight exists to catch.

After a passed preflight, the charter's latitude applies unchanged. Minor inline edits and scratch scripts are orchestrating; substantial writing goes only to contained seats.

## 3. The claim

Assigning the issue and re-reading it is not a claim, because two runs under one account both see a green re-read. The mutual exclusion is a tag on origin, created server-side, and `scripts/issue-claim.mjs` owns it.
...
2. **Adversarial minimum.** At least one review seat is prompted to refute the change, not to summarize it. A finding backed by a failing test it wrote is demonstrated rather than argued, so it skips adjudication and its fix inherits the test as the regression guard. A test can still encode the wrong requirement, so a fixer that disputes the test reports that instead of satisfying it. Prose refutation stays the minimum, since design flaws and missing coverage aren't demonstrable; the fast lane is an incentive, not a gate.
3. **Security visibility.** Report a refused, dead or errored security seat to the human in the final journal as an unavailable security review. No findings from a seat that never ran is absence of evidence. Retry across families before declaring it.
...
6. **Termination on evidence, not counters.** Convergence is risk-tiered:
   ...
   - breadth backstop: churn tripwires focus review on the file where fixes keep producing findings, and depth there isn't coverage elsewhere. The final pass before convergence sweeps the whole diff at file granularity and lists what it read and what it skipped; any file no reviewer has named since the last fix round is a gap. A bridge review returns only the validated findings array, so it cannot carry a coverage list. Continue that same review job, ask in plain text for the files read and skipped plus the verdict, and read the continuation for those only. The findings stay what the validated result said (§8).
...
7. **Containment.** No two seats hold the same file at once, and staging is repo-global even when the edits are disjoint, so a fan-out of parallel fixers serializes on the index unless each one stages only its own paths. Every seat already receives the charter's seat contract, native or delegated, so your own prompt carries the worktree path, the checkpoints and the task, and no contract text.
...
- **Standing consult**: when torn at a judgment point such as synthesis, triage or adjudication, ask the other family for a decorrelated second opinion before deciding. Two-key dismissal: a medium-or-higher finding is dismissed as noise only when both families agree it is.
...
- **Red team**: each family tries to break the other's implementation. Route demonstrable claims through the fast lane of invariant 2, because "prove it or drop it" beats a prose argument about severity.
...
**Trust-model forks.** A trust-model fork is anything that sets the posture of a trust boundary, such as who may reach what, what an unattended tool will read or publish, or where authority ends. Ask it on every host, whatever asking costs. A coherent trust model reads as intentional, so the review fabric ratifies a guessed posture rather than contesting it and the whole review budget goes on agreeing with itself. Unanswered, the run does not continue: take the conservative posture (confine, refuse, least reach), label `needs-human`, write the durable journal entry, and stop.

**The suspension protocol**, where asking ends the turn. Finish every mutation first: commit, push, the journal event, the label, the escalation notice. Then, immediately before asking, read the anchors: the AC snapshot digest, the head sha and the issue's `updatedAt`. Reading them earlier lets your own checkpoint expire your own question. Ask one question, up to four numbered options, the recommendation first, each option carrying its consequence in a line.
