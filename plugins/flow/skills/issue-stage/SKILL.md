...
A failed preflight stops the run with exactly three writes: the `needs-human` label, one comment naming what failed (the drift status, the unreadable contract path, or a workspace boundary the worktree would fall outside, with the directory to relaunch from), and the escalation notice of §9. Nothing else moves, and no inline edits happen for the rest of the run. Writing the code yourself because no seat can be spawned is the fallback the preflight exists to catch.
...
- `refused`: nothing of this run's is left anywhere; the executor confirmed every cleanup, and `retained` is empty by construction. `reason` says why, and the executor's header lists every reason with its fix; `live-run` means another run owns this issue and `found` says what it saw, so surface it and stop. The issue is re-read under the tag, so `issue-closed`, `not-ready` and `blocked` can also arrive at `phase: acquired` after a confirmed abandon. Fix what it names or route the human.
- `held`: another run held the tag before this run pushed anything (`phase: pre-acquire`, `retained` empty). Stop. A hold that was only observed after this run's own push is ambiguous and comes back as `unknown` with `reason: acquire-ambiguous` instead.
- `unknown`: an operational failure, or a cleanup the executor could not confirm. `reason` names it: `issue-unreadable`, `repo-unreadable`, `scan-unreadable`, `acquire-unknown`, `acquire-ambiguous`, `acquire-not-created` (the tag push failed and the re-read proved no tag exists; origin, a local hook, or the transport refused it, and the detail carries git's own line), `worktree-add`, `push`, `issue-edit`, `issue-edit-unconfirmed` (the label move ran but the re-read did not show OPEN, `in-progress`, no blocking label, and this account as assignee), `release`. Any post-acquire refusal (`live-run`, `issue-closed`, `not-ready`, `blocked`, `worktree-add`, `push`) also arrives as `unknown` under its own reason when the cleanup could not be confirmed; `retained` says what may be left. `phase` says how far it got: `pre-acquire` means nothing was mutated; `acquired` means the tag may exist on origin; `published` means the branch reached origin, which is the marker every other run scans for, and the tag stays until a human settles the issue's state. `retained` lists what may still exist: `claim-tag`, `worktree`, `local-branch`, `remote-branch`. `phase` is a lower bound (a push whose outcome could not be read back stays `acquired` while `retained` names `remote-branch`); `retained` is the list to act on. Report `reason`, `phase` and `retained` verbatim and stop. Never read `unknown` as "somebody else has it", and never delete or replace a tag this run did not create: an agent that can break its own locks does not have a lock. A stale tag is surfaced to the human with the branch and PR state it guards.
...
The native leg is your own family and the bridge leg's `family: other` floor puts it in the other one. Launch the native leg first, then run the bridge attached, so both sheets land in the same turn.
...
**Trust-model forks.** Anything that sets the posture of a trust boundary - who may reach what, what an unattended tool will read or publish, where authority ends - is asked on every host, whatever the binding costs. The review fabric ratifies a plausible trust ruling rather than contesting it, because a coherent trust model reads as intentional, so a guessed posture ships with its whole review budget spent agreeing with itself. Unanswered, the run does not continue: take the conservative posture (confine, refuse, least reach), label `needs-human`, write the durable journal entry, and stop.
...
**Fan-out medium.** Direct Agent calls when a stage is adaptive or small, at the session's effort. When the fan-out is deterministic, wide, and worth resume plus a progress readout, or when a native seat must run at the effort its binding names, a short ad-hoc script under the `Workflow` allowance: plain JavaScript, no TypeScript syntax, and no `Date.now()`, `Math.random()` or argless `new Date()`, because resume replays the script and those three break it. `TaskOutput` reads a seat back; `TaskStop` ends one. A bridge call inside a script, or one that must not block this thread, rides the transport seat the profile binds under `sub-seat`; the envelope it returns is the tool result, read exactly as a direct call's.
...
**Fan-out medium.** None. A deterministic fan-out is a batch of native spawns issued in one turn, and the verification between batches is git: read `git -C <wt> log` and `git -C <wt> status` before issuing the next batch. Size the batch to what you can check that way. A bridge call that must not block the parent is a detached call polled with `delegation_status`; this host binds no transport seat, because `spawn_agent` narrows nothing.
