...
Everything up to `## Host mechanics` is the same on every host; read your host's subsection there before step 1. Picking a seat, asking the human a question and reaching the other model family are the charter's, and whether asking ends the turn decides which forks get asked at all (§9). This file names the shape of a seat it wants, never a model.
...
- `unknown`: an operational failure, or a cleanup the executor could not confirm. `reason` names which one, and the executor's header lists them all with their fixes; any post-acquire refusal (`live-run`, `issue-closed`, `not-ready`, `blocked`, `worktree-add`, `push`) also arrives here under its own reason when the cleanup could not be confirmed. `phase` says how far it got: `pre-acquire` means nothing was mutated, `acquired` means the tag may exist on origin, and `published` means the branch reached origin, which is the marker every other run scans for, so the tag stays until a human settles the issue's state. `phase` is a lower bound - a push whose outcome could not be read back stays `acquired` while `retained` names `remote-branch` - so `retained` (`claim-tag`, `worktree`, `local-branch`, `remote-branch`) is the list to act on. Report `reason`, `phase` and `retained` verbatim and stop. Never read `unknown` as "somebody else has it", and never delete or replace a tag this run did not create: an agent that can break its own locks does not have a lock. A stale tag goes to the human with the branch and PR state it guards.
...
7. **Containment.** No two seats hold the same file at once, and staging is repo-global even when the edits are disjoint, so a fan-out of parallel fixers serializes on the index unless each one stages only its own paths. That constrains how you compose the fan-out as much as what the seats are handed. Every seat already receives the charter's seat contract, native or delegated, so your own prompt carries the worktree path, the milestones and the task, and no contract text.
   A **bridge writer** (a cross-family job at workspace write, on either host) edits and cannot commit: the delegation sandbox grants write on the job's workspace and nowhere else, this stage works in a linked worktree whose object store and refs live in the parent repository's git directory outside that grant, and network is off, so `git commit` and `git push` fail there by design. Say that in its task text, with the adaptation: its scope and milestone discipline governs the seat's EDITS, and the commit and report obligations move to you. When it comes back, do invariant 10's read yourself, then stage ONLY that seat's files by explicit path and commit with the seat named in the message body. Never `git add -A` after a bridge seat; the worktree may be holding a sibling's work. That sandbox also leaves local destruction open - `git checkout .` or `git clean -f` inside the worktree throws away a sibling's uncommitted work, which the git guard denies natively and nothing denies there - so never point a bridge writer at a worktree holding another seat's uncommitted work.
...
10. **Seat reports are claims.** Before acting on any completion or blocker report, run `git -C <wt> log` and `git -C <wt> status` and compare them against what the seat says it did. A seat that stopped mid-task gets a nudge carrying that verified state, which commits exist and what the tree holds, so it cannot re-litigate what is already done; your host's subsection says how a running seat is reached and what to do instead when it cannot be. A seat that fabricates twice is re-run on a stronger model, journaled as an event.
    A reading runs around every review seat too, for a different reason, and a status line plus a diff cannot do that job. A reviewer is read-only by its prompt and by the hooks, and on any host that leaves a shell in the seat it is not read-only by mechanism. Untracked files are where a status-and-diff check goes blind: `?? path` reads exactly the same before and after that file's bytes change, and `git diff` never carried its content. So take the tree snapshot, `node <plugin-root>/scripts/tree-snapshot.mjs <wt>`, the same four digests the prep stage takes at its entry, once before the review seat spawns and once when it returns. Any difference and the review does not count as covering the diff that ships: stop, name the path that moved, and find out what wrote it. A review of a tree that changed underneath it is a review of nothing that shipped.
...
**Fan-out medium.** Direct Agent calls when a stage is adaptive or small. When the fan-out is deterministic, wide and worth resume plus a progress readout, or when a seat has to run at an effort a bare Agent call cannot set, write a short ad-hoc script under the `Workflow` allowance: plain JavaScript, no TypeScript syntax, and no `Date.now()`, `Math.random()` or argless `new Date()`, because resume replays the script and those three break it.
