...
6. **Termination on evidence, not counters.** Convergence is risk-tiered:
   ...
   - breadth backstop: churn tripwires concentrate the fabric on the file that fights back, and depth there isn't coverage elsewhere. The final pass before convergence must sweep the whole diff at file granularity and list what it read and what it skipped; any file no reviewer has named since the last fix round is a gap. That list arrives in the review's summary output, beside the structured findings, and the summary is what you read for coverage and for the verdict, nothing else. The findings themselves still come only from the validated structure, which is what §8's rule against parsing review prose governs: reading a summary for which files a seat opened is not reading prose for defects. Churn depth and closing breadth are separate obligations.
