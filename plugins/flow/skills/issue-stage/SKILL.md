...
- The canonical seat contract at `agents/seat-contract.md` must be readable. An unreadable contract is not a formatting problem, it is a spawn prompt going out with no contract in it.
...
Existing-run detection is wider than the tag. Also check worktrees, `issue-N-*` branches, and open PRs naming the issue. If any of them turns up a live run, surface it rather than double-running.
