...
The bridge has its own check, later and mechanical. `canonicalWorkspace` in the delegation service decides which `cwd` a delegation job may run in, and it accepts a path two ways: the path resolves inside the session's roots, or an approved repository registered it as a linked worktree, which takes two proofs - the path's common git directory resolves inside a root, and that repository's own `git worktree list --porcelain` names the worktree. The pointer alone proves nothing, because a `.git` file is caller-writable and any directory can claim to belong to an approved repository. A path with neither proof fails closed with `OUTSIDE_ROOTS`. Creating the worktree is what completes the second proof, so this rule cannot run at preflight and every bridge call runs it afterwards against the real path. Passing it says a delegation job may run there; it never says your own shell or a native seat can write there, and those two answers come apart exactly when the host's roots diverge (see the Codex subsection).
...
A failed preflight stops the run, and the stop is not silent. The terminal escalation is the single exemption to the ban above, and it is the whole of it: the `needs-human` label, one comment naming what failed (the drift status, or the capability id and dimension, or the unreadable contract path), and the escalation notice of §9. Nothing else moves. No assignment, no `in-progress`, no claim tag, no branch, no worktree, no spawn, and the conductor's own minor-inline-edit latitude stays void for the rest of the run. Writing the code yourself because no seat can be contained is the named forbidden fallback: it is the exact failure the preflight exists to catch, and it is the one that looks most like progress from the inside.
...
Acquire through `node <plugin-root>/scripts/issue-claim.mjs acquire <N>`. It pushes `refs/tags/flow-claim-issue-N` without force and classifies the porcelain result, and its verdict is the whole answer:
...
State the assurance and move on. This is never a per-run trust question to the human, and two `contract` answers on a native writer are the posture that was settled at design time, not a question to re-open per run. Do not round a `contract` up to `mechanism` because the seat has never misbehaved.
...
1. **Decorrelation.** Every diff that ships is reviewed by at least one seat from a different model family than the one that wrote it. A family reviewing itself is a correlation failure, whichever family it is and whichever host is conducting - decorrelation is a rule about seat assignment, not about topology. Read the writer's family off the seat that produced the code, then pick the reviewer from the other one; a PR holding both native-written and bridge-written work needs one of each, judged per diff and not per run. Your host's subsection says how a seat of each family is reached and what a wider fabric costs there.
...
6. **Termination on evidence, not counters.** Convergence is risk-tiered:
   ...
   - breadth backstop: churn tripwires concentrate the fabric on the file that fights back, and depth there isn't coverage elsewhere. The final pass before convergence must sweep the whole diff at file granularity and list what it read and what it skipped; any file no reviewer has named since the last fix round is a gap. A native seat's report is prose already, and invariant 10 says how you check it against the tree. A bridge review has no prose to read: the job validates its whole final message against the closed findings schema, so a clean pass is an empty findings array and the coverage list has nowhere to ride. So continue that same review job with `delegation_continue` and ask one question, the complete list of files read and files skipped plus the verdict, in plain text. Read the continuation for coverage and the verdict and for nothing else. The findings stay exactly what the validated structure of the original result said, which is what §8's rule against parsing review prose protects. Churn depth and closing breadth are separate obligations.
...
The two families MUST differ. Two seats out of one family is one opinion said twice. Launch the native leg first, then run the bridge attached, so both sheets land in the same turn. Your host's subsection pins the models and the efforts. Which family is the outside opinion flips with the host, so take it from there rather than from habit.
...
- **Widen to three legs** by adding a taste leg, on the model the charter's table ranks highest for taste: a new subsystem, a public API, taste-heavy work. Or when the pair disagrees hard, which is a signal to widen rather than adjudicate thin.
...
The bridge is reached through the delegation tool the charter names, and what it costs differs by host. On one the outside family is flat-rate, so every pattern below turns from "can we afford it" into "does it help". On the other every cross-family review is metered. Your host's subsection carries that note, and the review is mandatory either way - decorrelation is not a budget line.
...
Delivery semantics are role language, not a preference. Attached delivery keeps the job's progress visible inside the call, and that is what a normal seat wants. Detached delivery is for a job that would otherwise hold the turn for longer than the conductor can afford to wait - a long review, a deep design leg - and it is legitimate: retain the job id and read it back with the delegation status and result tools, because a detached job you never read is an UNKNOWN you invented on purpose.
...
**Reaching a running seat.** `SendMessage`, carrying conductor-verified state read from git first. `TaskOutput` reads its stream when the final message is the thing in doubt.
...
**Seat ladder.** `mechanical`: `sonnet-5` at medium effort. `standard`: `opus-5` at high effort, the default. `hard`: `opus-5` at xhigh effort. Bulk sweeps go to `gpt-5.6-luna` through `delegate_to_codex`, at maximum effort when the sweep needs depth; that is the cheap-depth seat and never the decorrelation seat.

### Codex

**Argument.** There is no slash command; this host ignores a plugin's `commands/` directory. The number comes from the human's message naming the plugin's `issue-stage` skill or asking in words to run an issue: a bare integer or `#N` in that request is the subject, and a message that mentions `#N` while describing something else suspends the turn to ask which. `agents/openai.yaml` sets `allow_implicit_invocation: false`, but that suppression is unverified on this host's plugin-loader path, so do not lean on it: the human is the gate. There is no per-skill tool allowance here; the session's sandbox and approval policy apply as they are, to you and to every native child.

**Workspace boundary.** Three paths matter, and this host lets two of them diverge. The bridge root is the launch shell's PWD: `mcp-client-roots` reads `supported: false` with assurance `mechanism`, the Codex MCP client advertises no roots, so the delegation server falls back to `process.env.PWD`, and that is the root every delegation call validates a job's `cwd` against. The session root is the cwd this session actually runs in, which `codex -C` sets; it is the sandbox root, your own writes obey it, and a native `spawn_agent` child inherits it unchanged. After a plain launch the two are the same path. `cd parent; codex -C repo` splits them, and the sibling worktree under `parent` then satisfies every delegation check while no native seat can write a byte in it: preflight goes green and the run dies at `git worktree add`. So read the session root from the session's own cwd, never from the environment's PWD, and treat diverged roots as their own stop. The intended worktree sits inside the session root only when the operator launched from the directory HOLDING the repository with no `-C`; launched at the repository root, the run cannot create its worktree at all, because `git worktree add` writes outside the sandbox. Say that at preflight, name the directory to relaunch from, and stop. Do not claim the issue first, and do not move the worktree inside the repository to get around it.
...
Read a `supported: false` entry carrying `assurance: mechanism` for what it is: the absence was verified in the provider source, so the entry is trustworthy and the thing it describes is genuinely not there. What is left holding that dimension is the spawn prompt, and a prompt is `contract`. Two `contract` answers on this class is the posture settled at design time.
...
- descendants: `mechanism`. Service policy rejects `NESTED_DELEGATION`, and the delegated-worker policy denies a shell command that launches a provider, including one hidden behind an interpreter or a second shell.
...
**Seat ladder.** `mechanical`: `gpt-5.6-luna` at high effort. `standard`: `gpt-5.6-sol` at medium effort, the default. `hard`: `gpt-5.6-sol` at high effort. Sol at medium costs little more than Luna at high, so take the higher rung when torn. Bulk sweeps go to `gpt-5.6-luna`, at maximum effort when the sweep needs depth. A Claude-family writer at any rung is `delegate_to_claude` at `access: "workspace-write"`, a `bridge-writer` in the ledger.
