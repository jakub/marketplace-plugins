...
- Every tool declares the shape it returns, and the result envelope is validated before it is sent and again on arrival.
- The job record is readable as resources as well as through tools.
- An approval a provider asks for reaches the human through the session that started the job, when that session can render a form. Flow never grants one on its own.
...
Every tool declares an `outputSchema`. The job tools return `{ ok, job?, error? }`, `delegation_events` returns `{ ok, events?, error? }`, and doctor returns its check tree behind `{ ok }`. A success is `{ ok: true, job }`; an attached job that ended badly is `{ ok: false, job }` with `isError` set; a rejected request is `{ ok: false, error }` with `isError` set. All three fit the declared shape, because a client that validates every structured result against it, as MCP Client 1.30.0 does, must find an error result valid too. The SDK validates a success against the schema before it is sent, so a worker regression that drops a field or invents a status is a protocol error and never a value a model reads. The envelope's `error.kind` is enumerated from `ERROR_KINDS` in `contracts.mjs`, which the Codex smoke checks against every kind the source throws. `node dist/delegation.mjs schema envelope` prints the envelope's JSON Schema, for a caller that composes a schema of its own around it, such as a workflow script spawning the transport seat.

## Resources

The job record is also readable as MCP resources, for a client that lists and reads them. `flow://jobs` is this route's jobs, newest first, as envelopes. Three templates cover one job: `flow://jobs/{jobId}` is its envelope, `flow://jobs/{jobId}/events` is its ordered journal (the first 1000 events), and `flow://jobs/{jobId}/capabilities` is the target controls it ran under and whether it could ask the human. All are `application/json`, all run the route and workspace-visibility checks the tools run, the listings included (the database is shared across workspaces, so `flow://jobs` and the template listings are filtered to jobs inside the client's roots), and a read that fails unexpectedly is logged owner-side and reported as `INTERNAL`, because a resource error crosses the same redaction boundary a tool result does. State is read and mutations are called: the read tools stay, because a client without resources loses nothing, and no resource subscribes; progress notifications remain the live channel.
...
The approval fork asks first when it can. A job started by an attached call from a client that advertises `elicitation.form` carries `elicitation: true` in its envelope. When its provider asks to run a command or change a file, the worker renders the request whole: the command with its working directory, the item's full path list for a file change (read from the item notifications that preceded the request), or the tool with the host's own title and its whole input on the Claude route. A request that cannot be shown whole, because the command or input is longer than 4,000 characters, the file change has no item on record, or the approval is a network or stdin kind with no command, is declined unasked and journaled as `approval.undisclosed`: a form that hides part of the action approves something the human did not see. A showable request is parked in the store (`approval.requested`, the summary and never the raw request), and the server process, the only one holding the MCP session, sends `elicitation/create` with a two-value form. The attached wait loop does not await the form; it keeps reading the job, the caller's abort signal, and the terminal state, and the form races that same signal. An explicit `accept` is written back (`approval.decided`) and the worker answers the provider with it (`approval.granted`); anything else is a decline: a declined or dismissed form, a client error, or the 240-second window closing, which sits under the 420-second stall ceiling. A decline is journaled as `approval.denied` with `asked: true` and ends the job as `awaiting_approval` exactly as an unasked one does. A permissions request is never put to the human, because granting it would widen the sandbox the doctor proved; a detached job is never asked, because nobody is waiting to answer; a cancel on record beats an accept, checked by the worker before and after it reads the decision and enforced inside the store's write lock, so a cancel that lands before the human's accept turns that accept into a decline. Flow still grants nothing on its own: the form is the only path by which a job gains an approval it did not start with.
