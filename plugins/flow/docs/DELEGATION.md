...
Every tool declares an `outputSchema`. The job tools return `{ ok: true, job: <envelope> }`, `delegation_events` returns `{ ok: true, events: [...] }`, and doctor returns its check tree behind `{ ok }`. The SDK validates a result against the declared schema before it is sent, so a worker regression that drops a field or invents a status is a protocol error and never a value a model reads; a client that follows the specification validates it again on arrival. An error result is `{ ok: false, error }` with `isError` set and is not validated against the job shape. The envelope's `error.kind` is enumerated from `ERROR_KINDS` in `contracts.mjs`, which the Codex smoke checks against every kind the source throws. `node dist/delegation.mjs schema envelope` prints the envelope's JSON Schema, for a caller that composes a schema of its own around it, such as a workflow script spawning the transport seat.

## Resources

The job record is also readable as MCP resources, for a client that lists and reads them. `flow://jobs` is this route's jobs, newest first, as envelopes. Three templates cover one job: `flow://jobs/{jobId}` is its envelope, `flow://jobs/{jobId}/events` is its ordered journal (the first 1000 events), and `flow://jobs/{jobId}/capabilities` is the target controls it ran under and whether it could ask the human. All are `application/json`, all run the route and workspace-visibility checks the tools run, and a read that fails unexpectedly is logged owner-side and reported as `INTERNAL`, because a resource error crosses the same redaction boundary a tool result does. State is read and mutations are called: the read tools stay, because a client without resources loses nothing, and no resource subscribes; progress notifications remain the live channel.
...
The approval fork asks first when it can. A job started by an attached call from a client that advertises `elicitation.form` carries `elicitation: true` in its envelope. When its provider asks to run a command or change a file, the worker parks the request in the store (`approval.requested`, with a bounded public summary and never the raw request), and the server process, the only one holding the MCP session, sends `elicitation/create` with a two-value form. An explicit `accept` is written back (`approval.decided`) and the worker answers the provider with it (`approval.granted`); anything else is a decline: a declined or dismissed form, a client error, or the 240-second window closing, which sits under the 420-second stall ceiling. A decline is journaled as `approval.denied` with `asked: true` and ends the job as `awaiting_approval` exactly as an unasked one does. A permissions request is never put to the human, because granting it would widen the sandbox the doctor proved; a detached job is never asked, because nobody is waiting to answer; a cancel arriving mid-wait is a decline. Flow still grants nothing on its own: the form is the only path by which a job gains an approval it did not start with.
