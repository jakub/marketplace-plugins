...
The reset is refused while an older database still holds a live job. A detached worker and its provider outlive the MCP process that started them, so an upgrade can land in the middle of a run. A job row is not only history: a workspace-write job holds the lease on its worktree, and dropping the row would release that lease without proving the provider dead, leaving the next job free to claim a worktree something else is still writing to. Flow counts the rows whose `status` is `queued`, `starting`, `running`, `reconciling`, `awaiting_approval` or `quarantined`, and if any exist it throws `STORE_UPGRADE_BLOCKED` naming the count, leaving the file exactly as it found it. `awaiting_approval` is terminal in this version, but an older row carrying that status was written by code whose meaning of the word is gone, so it counts as live. The operator lets those jobs finish, or cancels them with the previous Flow version, and then upgrades. Doctor's `database` check reports the same kind, because a store that will not open is otherwise indistinguishable from any other database failure. A legacy database holding only terminal rows resets as before. `status` is the only column this count reads: it is the one column every earlier schema has. The `jobs` table records the request, route, canonical working directory, immutable review SHAs, model settings and limits, native session and turn IDs, heartbeat, provider process identities, result, error, usage, and parent job. The public envelope keeps the existing `threadId` field for both providers. For Claude that value is the native session ID.
