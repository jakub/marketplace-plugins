...
console.log('\na run already live on the issue')
{
  ...
  check('and names the branch it found', (r.json?.live ?? []).some((hit) => hit.ref === `refs/heads/feat/issue-${ISSUE}-someone-elses-run`), r.stdout)
...
}
{
  ...
  check('and names the branch that appeared', (r.json?.live ?? []).some((hit) => hit.ref === rival), r.stdout)
