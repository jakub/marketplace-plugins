...
A copy of the taxonomy that is legal except in one place: `mini-triage` still describes its
exit as a slash command, so the string one host types would be published into GitHub label
metadata every other reader sees. The label smoke runs its checker over this file and requires
that one problem and no other. Nothing loads it at runtime.

## Taxonomy (the state machine)

| label | lane | color | description (verbatim) | set by | cleared by |
|---|---|---|---|---|---|
| `mini-triage` | intake | `fbca04` | Untriaged intake; exits only through /flow:prep | human | the prep stage |
