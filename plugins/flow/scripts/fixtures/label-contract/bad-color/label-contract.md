...
A copy of the taxonomy that is legal except in one place: `mini-ready` carries `0e8a1g`, which
reads like a hex color and is not one. It sits alone in its lane on purpose, so the one-lane-
one-color rule stays quiet and the color check is the only thing that can report it. The label
smoke runs its checker over this file and requires that one problem and no other. Nothing loads
it at runtime.
...
| `mini-ready` | staging | `0e8a1g` | Design-hardened per the contract; eligible for the issue stage | the prep stage | the issue stage |
